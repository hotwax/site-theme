...
.widget-liquid-right .widget-content .row_options .image_select .cws_img_select_wrap').on('click', function(el){
	...
	function processMbInputOptions (el, params, bIsAssign, bToggleHide) {
		...
		if ( undefined !== el.getAttribute('data-options') && el.getAttribute('data-options') &&  ( ( !bIsAssign && !bDisabled ) || (bIsAssign && !bToggleHide && !bDisabled) ) ) {
			if (('checkbox' === el.type && bIsAssign) || ('radio' === el.type && !el.checked) ) {
				return;
			}
	...
	jQuery(document).ajaxSuccess(function(e, xhr, settings) {
		if (undefined !== settings.data) {
			...
			if (action && 'save-widget' === action[1] && (!isdel || '1' !== isdel[1]) ) {
	...
	function initSelectImage(parent) {
		jQuery(parent).find('a.pb-gmedia-cws-pb').on('click', function(e) {
			...
			var media_title = wp.media.view.l10n.addToGalleryTitle;

			if (selection) {
				state = 'gallery-edit';
				media_title = wp.media.view.l10n.editGalleryTitle;
			}
		...
		jQuery(parent).find('a.pb-media-cws-pb').on('click', function() {
   		...
   		window.send_to_editor = function(html) {
			...
			if (undefined !== this.dataset.type) {
				wp.media.editor.open(this, {
					library: {type:this.dataset.type},
				});
			} else {
				wp.media.editor.open(this);
			}
