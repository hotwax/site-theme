<?php
function prospect_sc_blog ( $atts = array(), $content = "" ){
	$out = "";
	$defaults = array(
		'title'									=> '',
		'title_align'							=> 'left',
	...
	$def_post_layout = prospect_get_option( 'def_blog_layout' );
	...
	$post_def_hide_meta = prospect_get_option( 'def_post_hide_meta' );
	...
	echo "<section id='$section_id' class='posts_grid {$post_type}_posts_grid posts_grid_{$layout} posts_grid_{$display_style}" . ( !empty( $el_class ) ? " $el_class" : "" ) . "'>";
		...
		echo "<div class='prospect_wrapper'>";
			echo "<div class='" . ( $is_carousel ? "prospect_carousel" : "prospect_grid" . ( is_numeric( $layout ) ? " isotope" : "" ) ) . "'" . ( $is_carousel ? " data-cols='" . ( !is_numeric( $layout ) ? "1" : $layout ) . "'" : "" ) . ">";
		...
		echo "</div>";
		if ( $use_pagination ){
			prospect_pagination ( $paged, $max_paged, false );
		}
	echo "</section>";
	$out = ob_get_clean();
	return $out;
...
function prospect_get_special_post_formats (){
	return array( "aside", "link", "quote" );
}
function prospect_is_special_post_format (){
	global $post;
	$sp_post_formats = prospect_get_special_post_formats ();
	if ( isset($post) ){
...
function prospect_post_format_mark (){
	global $post;
	if ( isset( $post ) ){
		$pf = get_post_format ();
		$icon = "book";
...
function prospect_get_post_thumbnail_dims ( $eq_thumb_height = false, $real_dims = array() ) {
	$def_grid_atts = array(
					'layout'				=> '1',
					'sb_layout'				=> '',
				);
...
function prospect_post_posts_grid_posts ( $q = null ){
	if ( !isset( $q ) ) return;
	$def_grid_atts = array(
					'layout'				=> '1',
	...
	if ( $q->have_posts() ):
		...
		while( $q->have_posts() && $q->current_post < $post_count - 1 ):
			$q->the_post();
			prospect_post_posts_grid_post ();
		endwhile;
		wp_reset_postdata();
		ob_end_flush();
	endif;				
}
function prospect_post_posts_grid_post (){
	$def_grid_atts = array(
					'layout'				=> '1',
					'post_hide_meta'				=> array(),
				);
	...
	post_class( array( 'item', 'post_post', 'post_grid_post', (is_sticky(get_the_id()) ? "sticky-post ": '') ) );
	echo ">";
		echo "<div class='post_post_wrapper'>";
			if($floated_media){
				...
				prospect_post_posts_grid_post_media ();
			if($floated_media){				
				echo "</div></div>";	
			}
				echo "<div class='post_post_content_wrapper'>";
					prospect_post_posts_grid_post_title ();
					if ( !in_array( 'terms', $post_hide_meta ) ){
						prospect_post_post_terms ();
					}
					echo "<hr class='posts_grid_small_divider' />";
					prospect_post_posts_grid_post_content ();
					prospect_page_links ();			
...
function prospect_post_posts_grid_post_title (){
	$pid = get_the_id();
	$title = get_the_title();
	$permalink = get_the_permalink();
	$is_special_post_format = prospect_is_special_post_format();
	echo !$is_special_post_format && !empty( $title ) ?	"<h3 class='post_post_title post_title'><a href='$permalink'>" . $title . "</a></h3>" : "";	
}
function prospect_post_post_header (){
	$pid = get_the_id();
	...
	if ( !in_array( 'date', $post_hide_meta ) ){
		...
		if ( $first_word_boundary ){
			$header_content .= "<div class='date'>
				<span class='first_word'>DATE:   </span>" . $date . "
			</div>";
		}
	}
	/* Post Info */
	if ( !in_array( 'post_info', $post_hide_meta ) ){
		...
		$special_pf = prospect_is_special_post_format();
		if ( !empty($author) || $special_pf ){
			$header_content .= "<div class='info'>";
				$header_content .= "<div class='info_container'>";
					$header_content .= !empty($author) ? "<span class='first_word'>AUTHOR:   </span> $author" : "";
					$header_content .= $special_pf ? ( !empty($author) ? PROSPECT_V_SEP : "" ) . prospect_post_format_mark() : "";
				$header_content .= "</div>";
			$header_content .= "</div>";
		}
	}
	...
	if ( !empty( $header_content ) ){
		...
		echo "<div class='post_post_header'>" . $header_content;
		echo "</div>";		
	}
}

function prospect_post_posts_grid_post_media (){
	...
	$thumbnail_dims = prospect_get_post_thumbnail_dims( $eq_thumb_height, $real_thumbnail_dims );
	$thumb_media = false;
	$some_media = false;
	ob_start();
	?>
			...
			<?php
				switch ($post_format) {
					...
					case 'quote':
						...
						if ( !empty( $quote ) ) {
							echo prospect_testimonial_renderer( array(
								'thumbnail'			=> $thumbnail,
								'quote'				=> $quote,
							...
							if ($match){
								...
								foreach ( $image_srcs as $image_src ) {
									...
									<div class='pic'>
										...
										<?php if ($links_enable) { ?>
											<div class="links">
												<?php echo ('<a href="' . esc_url($image_src) . '" ' . ($carousel ? ' data-fancybox-group="' . $gallery_id . '"' : '') . ' class="fancy fa ' . ($carousel ? 'fa-picture-o fancy_gallery' : 'fa-plus') . '">'  . $figure_style . '</a>'); ?>											</div>
										<?php } ?>
									</div>
									<?php
								}
				...
				if ( !$some_media && !empty( $thumbnail ) ) {
					...
					echo "<div class='pic'>";
						...
						if ( $links_enable)  {
							echo "<div class='links'><a class='fancy fa fa-plus' href='$thumbnail'>$figure_style</a><a class='fa fa-share' href='$post_url'>$figure_style</a></div>";
						}
					echo "</div>";
					$thumb_media = true;
					$some_media = true;
...
function prospect_post_single_post_media (){
	$pid = get_the_id();
	$figure_style =  prospect_figure_style() ;
	$def_grid_atts = array(
					'layout'				=> '1',
						...
						if ($match){
							...
							foreach ( $image_srcs as $image_src ) {
								...
								<div class='pic'>
									...
									<div class="hover-effect"></div>
									<div class="links">
									<?php echo ('<a href="' . esc_url($image_src) . '" ' . ($carousel ? ' data-fancybox-group="' . $gallery_id . '"' : '') . ' class="fancy fa ' . ($carousel ? 'fa-picture-o fancy_gallery' : 'fa-plus') . '">'  . $figure_style . '</a>'); ?>	
									</div>
								</div>
								<?php
							}
	...
	$GLOBALS['prospect_single_post_floated_media'] = $thumb_media && !$crop_thumb;
}

function prospect_post_posts_grid_post_content (){
	global $post;
	global $more;
	$id = get_the_ID();
	...
	$grid_atts = isset( $GLOBALS['prospect_posts_grid_atts'] ) ? $GLOBALS['prospect_posts_grid_atts'] : $def_grid_atts;	
	extract( $grid_atts );
	$chars_count = !empty( $chars_count ) ? $chars_count : prospect_post_posts_grid_get_chars_count();
	...
	$read_more = $terms_hidden ? esc_html__( 'Read More', 'prospect' ) . " <i class='fa fa-angle-right'></i>" : esc_html__( 'READ MORE', 'prospect' ) . " <i class='fa fa-long-arrow-" . ( $is_rtl ? "left" : "right" ) . "'></i>";
...
function prospect_post_posts_grid_get_chars_count() {
...
function prospect_post_post_terms (){
	$terms = $tags = $cats = "";
	if ( has_category() ) {
		ob_start();
		the_category ( "&#x2c;&#x20;" );
	...
	$terms .= !empty( $tags ) ? ( ( !empty( $tags ) ? PROSPECT_V_SEP : "" ) . $tags ) : "";
	if ( !empty( $terms ) ){
		echo "<div class='post_post_terms post_terms'>" . $terms;
		echo "</div>";
	}
}
