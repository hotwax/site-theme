...
class Prospect_Walker_Comment extends Walker_Comment {
	...
	public function start_el( &$output, $comment, $depth = 0, $args = array(), $id = 0 ) {
		...
		<div <?php comment_class( $parent_class ); ?> id="comment_<?php comment_ID() ?>">
			...
			<div id="comment_body_<?php comment_ID() ?>" class="comment_body clearfix">
				<?php
					if ( !empty( $avatar ) ){
						echo "<div class='avatar_section'>" . $avatar;
						echo "</div>";
					}
				?>
				<div class="comment_section">
					<div class="comment_meta comment_meta_data">
						...
						<?php edit_comment_link( esc_html__( 'Edit Comment', 'prospect' ) ); ?>
						<span class="comment_date">
							<?php
								comment_date();
								echo esc_html__( '&#x20;at&#x20;', 'prospect' );
								comment_time();
							?>
						</span>
					</div><!-- /.comment-meta -->
					<div id="comment_content_<?php comment_ID(); ?>" class="comment_content">
						...
						<em class="comment_awaiting_moderation"><?php esc_html_e('Your comment is awaiting moderation.', 'prospect'); ?></em>
						<?php else: comment_text(); ?>
						<?php endif; ?>
					</div><!-- /.comment-content -->
					<div class="reply clearfix">
						<?php $reply_args = array(
							'reply_text' => esc_html__( "REPLY", 'prospect' ) . " <i class='fa fa-long-arrow-right'></i>&#x20;",
							'depth' => $depth,
...
function prospect_comment_nav() {
	// Are there comments to navigate through?
	if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
	?>
	<div class="comments_nav clearfix">
		<?php
			$prev_link = get_previous_comments_link( "<i class='fa fa-angle-double-left'></i>&#x20;<span>" . esc_html__( 'Older Comments', 'prospect' ) . "</span>" );
			$next_link = get_next_comments_link( "<span>" . esc_html__( 'Newer Comments', 'prospect' ) . "</span>&#x20;<i class='fa fa-angle-double-right'></i>" );
			if ( !empty( $prev_link ) ) :
