<?php
function prospect_searchPostsGrid_post (){
	$pid = get_the_id();
	echo "<article id='post_post_{$pid}' class='item post post_post post_grid_post clearfix'>";
		
		prospect_searchPostsGrid_postMedia ();

		prospect_searchPostsGrid_postTitle ();

		prospect_searchPostsGrid_postContent ();
		
		prospect_searchPostsGrid_postTerms ();

		echo "<hr class='posts_grid_divider' />";											
	echo "</article>";	
}
function prospect_searchPostsGrid_postMedia (){
	$pid = get_the_id();
	...
	if ( has_post_thumbnail() ) {
		...
		$thumb_obj = cws_thumb( $thumbnail, array( 'width' => 150, 'height' => 150 ), false );
...
function prospect_searchPostsGrid_postTitle (){
	$search_terms = get_query_var( 'search_terms' );	
	if ( empty( $search_terms ) ){
		prospect_searchPostsGrid_postTitle_emptyQuery ();	
	}
	else{
		prospect_searchPostsGrid_postTitle_searchByTerms ();
	}
}
function prospect_searchPostsGrid_postTitle_emptyQuery (){
	$pid = get_the_id();
	$title = esc_html( get_the_title() );
	$permalink = esc_url( get_the_permalink() );
	echo !empty( $title ) ?	"<h3 class='post_post_title post_title'><a href='$permalink'>" . $title . "</a></h3>" : "";	
}
function prospect_searchPostsGrid_postTitle_searchByTerms (){
	$search_terms = get_query_var( 'search_terms' );
	$title = esc_html( get_the_title() );
	$permalink = esc_url( get_the_permalink() );
...
function prospect_postPostsGrid_getCharsCount() {
	$layout = "small";
	$number 	= 300;
...
function prospect_searchPostsGrid_postContent (){
	$search_terms = get_query_var( 'search_terms' );	
	if ( empty( $search_terms ) ){
		prospect_searchPostsGrid_postContent_emptyQuery ();	
	}
	else{
		prospect_searchPostsGrid_postContent_searchByTerms ();
	}
}
function prospect_searchPostsGrid_postContent_emptyQuery (){
	global $post;
	global $more;
	$id = get_the_ID();
	...
	extract( $grid_atts );
	$terms = prospect_searchPostsGrid_get_postTerms ();
	$content = $proc_content = $excerpt = $proc_excerpt = "";
	$read_more_exists = false;
	$read_more = empty( $terms ) ? esc_html__( "Read More ", 'prospect' ) . "<i class='fa fa-angle-right'></i>" : "<i class='fa fa-angle-double-right'></i>";
	...
	if ( !empty( $excerpt ) ){
		...
		if ( !empty( $content ) ){
			...
			$def_chars_count = prospect_postPostsGrid_getCharsCount();
...
function prospect_searchPostsGrid_postContent_searchByTerms (){
	global $post;
	$search_terms = get_query_var( 'search_terms' );
	$permalink = esc_url( get_the_permalink() );
	$terms = prospect_searchPostsGrid_get_postTerms ();
	...
	if ( !empty( $cont ) ){
		echo "<div class='post_content post_post_content post_posts_grid_post_content clearfix" . ( !empty( $terms ) ? " read_more_alt" : "" ) . "'>";
			...
			if ( empty( $terms ) ){
				$cont .= esc_html__( "Read More", 'prospect' ) . " <i class='fa fa-angle-right'></i>";
			}
			else{
...
function prospect_searchPostsGrid_get_postTerms (){
	$pid = get_the_id();
	$post_type = get_post_type( $pid );
	$terms = $tags = $cats = $pcats = $deps = $poss = "";
	if ( $post_type == 'cws_portfolio' ){
		$pcats = prospect_get_post_term_links_str( 'cws_portfolio_cat' );
...
function prospect_searchPostsGrid_postTerms (){
	$terms = prospect_searchPostsGrid_get_postTerms ();
	if ( !empty( $terms ) ){
		echo "<div class='post_terms post_post_terms post_terms'>" . $terms;
		echo "</div>";
	}		
}
?>