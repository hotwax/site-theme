				...
				if( have_posts() ){
					echo "<section id='$section_id' class='posts_grid search_posts_grid'>";
						echo "<div class='prospect_wrapper'>";
							echo "<div class='prospect_grid isotope'>";
								wp_enqueue_script ('isotope');
								$use_pagination = $max_paged > 1;
									while( have_posts() ) : the_post();									
										prospect_searchPostsGrid_post ();							
									endwhile;
									wp_reset_postdata();
							echo "</div>";
						echo "</div>";
					...
					echo do_shortcode( "[cws_sc_msg_box type='info' title='" . esc_html__( "No search Results", 'prospect' ) . "']" . esc_html__( "There are no posts matching your query", 'prospect' ) . "[/cws_sc_msg_box]" );
