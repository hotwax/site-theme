		...
		echo "<main id='page_content'>";
			while ( have_posts() ) : the_post();
				...
				echo "<article id='attachment_post_{$pid}' class='attachment_post post_single clearfix'>";
					...
					$thumbnail_dims = prospect_get_post_thumbnail_dims( false, $real_thumbnail_dims );
					...
					if ( !empty( $content ) ){
						echo "<div class='post_content attachment_post_content  attachment_single_post_content" . ( $floated_media ? " clearfix" : "" ) . "'>" . $content;
						echo "</div>";
					}
					prospect_page_links ();
				echo "</article>";
				...
				ob_start();
				$prev_link_text = is_rtl() ? "<span>" . esc_html__( 'Previous Image', 'prospect' ) . "</span>&#x20;<i class='fa fa-angle-double-right'></i>" : "<i class='fa fa-angle-double-left'></i>&#x20;<span>" . esc_html__( 'Previous Image', 'prospect' ) . "</span>";
				$next_link_text = is_rtl() ? "<i class='fa fa-angle-double-left'></i>&#x20;<span>" . esc_html__( 'Nexts Image', 'prospect' ) . "</span>" : "<span>" . esc_html__( 'Next Image', 'prospect' ) . "</span>&#x20;<i class='fa fa-angle-double-right'></i>";
