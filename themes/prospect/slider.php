<?php
	...
	$is_revslider_active = prospect_check_for_plugin( 'slider_revolution/revslider.php' );
	$slider_type = 'none';
	if ( is_front_page() ) {
		$slider_type = prospect_get_option( 'home_slider_type' );
		switch ( $slider_type ) {
			case 'img_slider':
				if ( is_page() ) {
					$slider_settings = prospect_get_page_meta_var( 'slider' );
					if ( isset( $slider_settings['slider_override'] ) && $slider_settings['slider_override'] ) {
						...
						$slider_shortcode = prospect_get_option( 'home_slider_shortcode' );
					}
				} else{
					$slider_shortcode = prospect_get_option( 'home_slider_shortcode' );
				}
				...
				break;
			case 'video_slider':
				$video_settings = prospect_get_option( 'video_section' );
				...
				if ( $overlay_type == 'color' ){
					...
					$gradient_rules = prospect_render_gradient_rules (
						array(
			...
			case 'stat_img_slider':
				$image_options = prospect_get_option( 'static_img_section' );
				extract( shortcode_atts( array(
					'static_img' => array()
				), $image_options));
